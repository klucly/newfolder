...
impl<'s> System<'s> for MouseHandleSystem {
    ...
    fn run(&mut self, (buttons, input, mut button_event_system, menu, transforms, balls, score, mut con, mut exit, mat_buffer): Self::SystemData) {
        let bot_buttons = [
            ButtonType::P1You,
            ButtonType::P1Bot,
            ButtonType::P1Mat,
                    ...
                    if point_in_rect(x, y, button.pos.x, button.pos.x + button.size.x, button.pos.y + button.size.y, button.pos.y) {
                        if bot_buttons.contains(&button.but_type) {
                            ...
                            match button.but_type {
                                ButtonType::Reset => WinningSystem::full_reset(balls, transforms, score, mat_buffer),
